...
# Unified adapter for all Uniswap V3 calculations
...
class UniswapV3LPEnv(gym.Env):
    ...
    def _calculate_fees(self, current_data: pd.Series, next_data: pd.Series) -> float:
        ...
        fees_usd = calculate_fees_whitepaper(
            ...
            fee_growth_start_0=fg_start_0,
            fee_growth_end_0=fg_end_0,
            fee_growth_start_1=fg_start_1,
            fee_growth_end_1=fg_end_1,
            active_percentage=active_liq,
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        f5 = np.log1p(current_data.get('tvl', 1000000)) / 20  # Log-normalized TVL
        f6 = np.log1p(current_data.get('volume_24h', 10000)) / 15  # Log-normalized volume
        f7 = current_data.get('liquidity', 1000000) / 1e6  # Liquidity depth
